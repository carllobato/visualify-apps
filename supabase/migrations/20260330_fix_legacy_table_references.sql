...
DROP FUNCTION IF EXISTS public.has_project_member_role(uuid, uuid, text[]);
...
CREATE OR REPLACE FUNCTION public.user_has_project_role(p_project_id uuid, p_roles text[])
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT public.has_project_member_role(p_project_id, p_roles);
$$;
