...
export type AppShellTwoFactorSetupProps = {
  totpAlreadyEnabled?: boolean;
  getSupabaseClient: () => AppShellSupabaseAuthClient;
};

export function AppShellTwoFactorSetup({
  totpAlreadyEnabled = false,
  getSupabaseClient,
}: AppShellTwoFactorSetupProps) {
  ...
  async function handleVerifySubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (verifyErr) {
        setErrorMessage(verifyErr.message ?? "Verification failed.");
        setPhase("setup");
        return;
      }
        ...
        <div className="space-y-4 border-t border-[var(--ds-border)] pt-4">
          ...
          <form onSubmit={(e) => void handleVerifySubmit(e)} className="space-y-3">
            <div className="max-w-sm">
              <Label htmlFor="app-shell-two-factor-verify-code" className="!mb-1">
                Authenticator code
              </Label>
              <Input
                id="app-shell-two-factor-verify-code"
                type="text"
                inputMode="numeric"
