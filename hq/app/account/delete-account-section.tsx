...
import { Button, Callout, Card, CardContent, CardHeader } from "@visualify/design-system";

/** Mirrors RiskAI `DeleteAccountSection`; calls `DELETE /api/account` then clears the session. */
export function DeleteAccountSection() {
  ...
  return (
    <>
      <Card
        variant="default"
        className="[border-width:var(--ds-border-width)] border-[var(--ds-border)] bg-[var(--ds-surface-elevated)]"
      >
      ...
      {open ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="hq-delete-account-title"
          aria-describedby="hq-delete-account-desc"
        ...
        >
          ...
          >
            <h3
              id="hq-delete-account-title"
              className="text-lg font-semibold tracking-tight text-[var(--ds-text-primary)]"
            >
              Delete account?
            </h3>
            <p id="hq-delete-account-desc" className="mt-2 text-sm text-[var(--ds-text-secondary)]">
