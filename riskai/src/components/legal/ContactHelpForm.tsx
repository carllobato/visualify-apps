...
/** Aligns with dashboard / settings: profile row first, then auth metadata. */
function contactPrefillDisplayName(user: User, profile: PublicProfileRow | null): string {
...
export function ContactHelpForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setEmailValue(email);
      setNameValue(contactPrefillDisplayName(user, profile));
      setPrefillLoading(false);
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (prefillLoading || didInitialFocus.current) return;
    didInitialFocus.current = true;
    if (nameValue.trim()) messageTextareaRef.current?.focus();
    else nameInputRef.current?.focus();
  }, [prefillLoading, nameValue]);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ name, email, message, source: "riskai-footer" }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="riskai-cf-name" className="mb-1.5 block text-[13px] font-medium text-foreground">
          Name
        </label>
        <input
          ref={nameInputRef}
      ...
      <div>
        <label htmlFor="riskai-cf-email" className="mb-1.5 block text-[13px] font-medium text-foreground">
          Email
        </label>
        <input
          id="riskai-cf-email"
      ...
      <div>
        <label htmlFor="riskai-cf-message" className="mb-1.5 block text-[13px] font-medium text-foreground">
          Message
        </label>
        <textarea
          ref={messageTextareaRef}
          id="riskai-cf-message"
          name="message"
          rows={4}
          required
          disabled={busy}
